...
class Wechat
{
    ...
    */
     ...
     * @param string $text
     */
    public function text($text='', $from = '', $to = '')
    {
...
<Nonce><![CDATA[%s]]></Nonce>
     ...
     * @param string $jsapi_ticket 手动指定jsapi_ticket，非必要情况不建议用
     */
    public function getOpenJsTicket($appid='',$jsapi_ticket=''){
        // if (!$this->access_token && !$this->checkAuth()) return false;
        ...
        $result = $this->http_get(self::API_URL_PREFIX.self::GET_TICKET_URL.'access_token='.$access_token.'&type=jsapi');
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
     ...
     * @return array|bool 返回签名字串
     */
    public function getOpenJsSign($url, $timestamp=0, $noncestr='', $appid=''){
        if (!$this->jsapi_ticket && !$this->getOpenJsTicket($appid) || !$url) return false;
        if (!$timestamp)
            $timestamp = time();
        if (!$noncestr)
            $noncestr = $this->generateNonceStr();
     ...
     * 注意：unionid字段 只有在用户将公众号绑定到微信开放平台账号后，才会出现。建议调用前用isset()检测一下
     */
    public function getOpenUserInfo($openid, $authorizer_access_token){
        if (!$authorizer_access_token) return false;
        $result = $this->http_get(self::API_URL_PREFIX.self::USER_INFO_URL.'access_token='.$authorizer_access_token.'&openid='.$openid);
        if ($result)
        {
            $json = json_decode($result,true);
            if (isset($json['errcode'])) {
