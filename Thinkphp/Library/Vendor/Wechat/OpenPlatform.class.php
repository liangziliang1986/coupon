...
class Wechat
{
    ...
    const OPEN_OAUTH_TOKEN_URL = '/sns/oauth2/component/access_token?';
...
<Nonce><![CDATA[%s]]></Nonce>
     ...
     * @param string $component_appid 第三方平台的appid
     * @return string
     */
    public function getOpenOauthRedirect($callback, $appid, $component_appid, $state='', $scope='snsapi_userinfo'){
        return self::OAUTH_PREFIX.self::OAUTH_AUTHORIZE_URL.'appid='.$appid.'&redirect_uri='.urlencode($callback).'&response_type=code&scope='.$scope.'&state='.$state.'&component_appid='.$component_appid.'#wechat_redirect';
    }


    /**@bill add
     * 通过第三方平台获取Access Token 同时获取到 openid
     * @return array {access_token,expires_in,refresh_token,openid,scope}
     */
    public function getOpenOauthAccessToken($appid, $component_appid, $component_access_token){
        ...
        $result = $this->http_get(self::API_BASE_URL_PREFIX.self::OPEN_OAUTH_TOKEN_URL.'appid='.$appid.'&code='.$code.'&grant_type=authorization_code&component_appid='.$component_appid.'&component_access_token='.$component_access_token);
        // echo self::API_BASE_URL_PREFIX.self::OAUTH_TOKEN_URL.'appid='.$appid.'&code='.$code.'&grant_type=authorization_code&component_appid='.$component_appid.'&component_access_token='.$component_access_token;die;
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
            ...
            $this->open_user_token = $json['access_token'];
            return $json;
        }
        return false;
    }
    ...
    /**
     * 通过code获取Access Token 同时获取到 openid
     * @return array {access_token,expires_in,refresh_token,openid,scope}
     */
...
{
    //opts数组保存应用id，应用secret和第三方平台的id；
    public function __construct ($opts) {
        $options = array(
            /*'appid' => C('WEIXIN_APPID'),
            'appsecret' => C('WEIXIN_APPSECRET'),*/
            'appid' => $opts['weixin_appid'],
            'appsecret' => $opts['weixin_appsecret'],
            );
        parent::__construct($options);
        $this->component_appid = $opts['weixin_open_appid'];
    }

    //用户点击引导链接后同意授权的回调地址，获取真正的授权码
    public function auth_callback () {
        $auth_code = $_GET['auth_code'];
        $expires_time = $_GET['expires_in'] + time();
        // writeArray('/home/wwwroot/weishidai/Runtime/Logs/Home/' . date('d-H-i-s') . 'auth_code.php', $auth_code);
        $authorizer_access_token = $this->get_authorizer_access_token($auth_code);
    }


    //获取授权者的令牌authorizer_access_token，用于调用公众号相应AIP，代替原来的access_token即可。
    public function get_authorizer_access_token ($auth_code = '') {
        //在redis中取
        $authorizer_access_token = unserialize($this->_cache->get('authorizer_access_token'));
        // $authorizer_access_token = null;
        // var_dump($authorizer_access_token['expires_time'] - time());die;
        if ($authorizer_access_token && $authorizer_access_token['expires_time'] > time()) {
            //直接返回
            return $authorizer_access_token['authorization_info']['authorizer_access_token'];
        } else if ($authorizer_access_token && ($authorizer_access_token['authorization_info']['authorizer_refresh_token'] || $authorizer_access_token['authorizer_refresh_token'])) {
            //如果有authorizer_refresh_token的话，就刷新
            ...
            $param = array(
                ...
                "authorizer_appid" => $authorizer_access_token["authorization_info"]["authorizer_appid"],
                "authorizer_refresh_token" => $authorizer_access_token["authorization_info"]['authorizer_refresh_token'],
            );
            $res = $this->http_post($url, json_encode($param));
            $json = json_decode($res, true);
            if ($json && !$json['errcode']) {
                $authorizer_access_token['expires_time'] = time() + 7000;
                $authorizer_access_token["authorization_info"]["authorizer_refresh_token"] = $json["authorizer_refresh_token"];
                $authorizer_access_token["authorization_info"]["authorizer_access_token"] = $json["authorizer_access_token"];
                $this->_cache->set('authorizer_access_token', serialize($authorizer_access_token));
            ...
            $param = array(
                "component_appid" => $this->component_appid,
                //@bill 这里第一次进入时，记得改回$auth_code;
                // "authorization_code" => $auth_code,
                "authorization_code" => 'QtHL91zG3dNVtSwGRh-BXMufps8SRVSXMlkVyyFVk9PKofOhSjpW382fxulT1pHe',
            );
            $res = $this->http_post($url, json_encode($param));
            //如果公众号绑定多个平台，可以使用结果集的func_info进行验证
            $json = json_decode($res, true);
            $json['expires_time'] = time() + 3300;
            $this->_cache->set('authorizer_access_token', serialize($json));
            // var_dump($json);
            return $json['authorization_info']['authorizer_access_token'];
        }
    }
    ...
    public function get_pre_auth_code () {
        $pre_auth_code = unserialize($this->_cache->get('pre_auth_code'));
        // var_dump($pre_auth_code);
        if ($pre_auth_code && !$pre_auth_code['errcode'] && $pre_auth_code['expires_time'] > time()) {
            ...
            $res = $this->http_post($url, json_encode($param));
            $json = json_decode($res, true);
            $json['expires_time'] = time() + 1500;
            $this->_cache->set('pre_auth_code', serialize($json));
            return $json['pre_auth_code'];
        }
    }
    

    //获取第三方平台令牌get_component_access_token，用于调用第三方平台信息的接口
    public function get_component_access_token () {
        $component_access_token = unserialize($this->_cache->get('component_access_token'));
        if ($component_access_token && !$component_access_token['errcode'] && $component_access_token['expires_time'] > time()) {
            ...
            $param = array(
                "component_appid" => $this->component_appid,
                "component_appsecret" => C('WEIXIN_OPEN_APPSECRET'), 
                "component_verify_ticket" => $this->_cache->get('component_verify_ticket'), 
            );
            $result = $this->http_post($url, json_encode($param));
            $json = json_decode($result,true);
            $json['expires_time'] = time() + 7000;
            $this->_cache->set('component_access_token', serialize($json));
            // var_dump($json);
    ...
    public function ticket () {
        $token = C('WEIXIN_OPEN_TOKEN');
        $encodingAesKey = C('WEIXIN_OPEN_ENCODINGAESKEY');
        ...
        $tem['msg_signature'] = $signature;
        $tem['timestamp'] = $timestamp;
        $tem['nonce'] = $nonce;
        $pc = new \WXBizMsgCrypt($token, $encodingAesKey, $appId);
        $msg = '';
        $errCode = $pc->decryptMsg($signature, $timestamp, $nonce, $from_xml, $msg);
        // writeArray('/home/wwwroot/weishidai/Runtime/Logs/Home/' . date('d-H-i-s') . 'errCode.php', $errCode);
        $array = (array)simplexml_load_string($msg, 'SimpleXMLElement', LIBXML_NOCDATA);
        $this->_cache->set('component_verify_ticket', $array['ComponentVerifyTicket']);
        echo 'success';
    }

    //获取授权公众号的信息
    public function get_authorizer_info () {
        $authorization_info = unserialize($this->_cache->get('authorizer_access_token'));
    ...
    //该API用于获取授权方的公众号的选项设置信息，如：地理位置上报，语音识别开关，多客服开关。具体选项名请查看'选项名和选项值表 ： https://open.weixin.qq.com/cgi-bin/showdocument?action=dir_list&t=resource/res_list&verify=1&lang=zh_CN&token=a2102d586508609113e10765d8d9cdb476cf506b&appid=wx0a9499c6ab10addc'
    public function  check_options_status () {
        $authorization_info = unserialize($this->_cache->get('authorizer_access_token'));
